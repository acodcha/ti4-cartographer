...
enum class Anomaly : uint8_t { AsteroidField, GravityRift, Nebula, Supernova };
...
const std::map<Anomaly, float> anomaly_scores{
  // If you have Antimass Deflectors researched, this is effectively an empty
  // system; if you do not, this is worse because it prevents movement and
  // therefore reduces your options. Therefore, this type of anomaly is
  // generally undesirable, except for the Clan of Saar which strongly prefers
  // this anomaly due to its faction technology.
  {Anomaly::AsteroidField, -0.75f},

  // Gives extra movement and therefore extra options, but destroys 30% of your
  // ships. Situationally useful, but dangerous.
  {Anomaly::GravityRift,   -0.25f},

  // Better defense, but greatly impairs movement, so generally undesirable.
  // However, the Empyrean strongly prefers this anomaly due to its faction
  // ability.
  {Anomaly::Nebula,        -0.5f },

  // Prevents movement and therefore reduces your options, so generally
