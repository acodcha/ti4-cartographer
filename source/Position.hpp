...
public:
  ...
  /// \brief Set of positions that are neighbors with this position, i.e.
  /// adjacent. These positions may or may not exist on the game board.
  std::set<Position> possible_neighbors() const noexcept {
    std::set<Position> positions;
    const int8_t maximum_azimuth_{maximum_azimuth(layer_)};
    const int8_t number_of_corners{
        static_cast<int8_t>(layer_ > 0 ? azimuth_ / layer_ : 0)};
    const int8_t azimuth_after_corner{
        static_cast<int8_t>(layer_ > 0 ? azimuth_ % layer_ : 0)};
    // Same layer.
    if (layer_ > 0) {
    ...
    if (layer_ > 0) {
      ...
      if (is_a_corner()) {
        ...
        positions.insert(
            {layer_minus_one,
             static_cast<int8_t>(azimuth_ / layer_ * layer_minus_one)});
      } else {
        // There are 2 inner layer neighbors.
        const int8_t inner_layer_azimuth_1{static_cast<int8_t>(
            number_of_corners * layer_minus_one + azimuth_after_corner - 1)};
        positions.insert({layer_minus_one, inner_layer_azimuth_1});
        const int8_t inner_layer_azimuth_2{static_cast<int8_t>(
            number_of_corners * layer_minus_one + azimuth_after_corner)};
    ...
    const int8_t outer_layer_azimuth_to_corner{
        static_cast<int8_t>(number_of_corners * layer_plus_one)};
    if (layer_ > 0) {
      if (is_a_corner()) {
        ...
        if (outer_layer_azimuth_to_corner > 0) {
          positions.insert(
              {layer_plus_one,
               static_cast<int8_t>(outer_layer_azimuth_to_corner - 1)});
        } else {
        ...
        positions.insert(
            {layer_plus_one, static_cast<int8_t>(outer_layer_azimuth_to_corner
                                                 + azimuth_after_corner)});
        positions.insert(
            {layer_plus_one, static_cast<int8_t>(outer_layer_azimuth_to_corner
                                                 + azimuth_after_corner + 1)});
      }
    } else {
      // In this case, the layer is 0, and the only neighbors are the 6
      // neighbors on layer 1.
...
private:
  ...
  /// Layer 3 contains the 18 tiles around those. This is the layer that
  /// contains the home systems in a regular 6-player game. Layer 4 is only used
  /// in 7-8 player games and contains 24 tiles.
  int8_t layer_{0};

  /// \brief Azimuth is the radial position around a layer.
  /// \details Azimuth 0 is the northernmost tile of the layer, and azimuth
  /// increases clockwise. The tiles of layer 1 can have azimuth 0 to 5. The
  /// tiles of layer 2 can have azimuth 0 to 11. The tiles of layer 3 can have
  /// azimuth 0 to 17. The tiles of layer 4 can have azimuth 0 to 23.
  int8_t azimuth_{0};
